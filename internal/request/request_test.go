...
func TestRequestLineParse(t *testing.T) {
	...
	r, err = RequestFromReader(strings.NewReader("POST /api/data HTTP/1.1\r\nHost: localhost:42069\r\nContent-Type: application/json\r\n\r\n"))
	require.NoError(t, err)
	require.NotNil(t, r)
	assert.Equal(t, "POST", r.RequestLine.Method)
	assert.Equal(t, "/api/data", r.RequestLine.RequestTarget)
	assert.Equal(t, "1.1", r.RequestLine.HttpVersion)
	...
	// Test: PUT Request
	r, err = RequestFromReader(strings.NewReader("PUT /resource HTTP/1.1\r\nHost: localhost:42069\r\n\r\n"))
	require.NoError(t, err)
	assert.Equal(t, "PUT", r.RequestLine.Method)

	// Test: DELETE Request
	r, err = RequestFromReader(strings.NewReader("DELETE /resource/123 HTTP/1.1\r\nHost: localhost:42069\r\n\r\n"))
	require.NoError(t, err)
	assert.Equal(t, "DELETE", r.RequestLine.Method)
	assert.Equal(t, "/resource/123", r.RequestLine.RequestTarget)

	// Test: Single part request line
	_, err = RequestFromReader(strings.NewReader("GET\r\n"))
	require.Error(t, err)

	// Test: HTTP/1.0 (Invalid version)
...
func TestRequestHeadersParse(t *testing.T) {
	...
	require.NoError(t, err)
	require.NotNil(t, r)
	assert.NotEmpty(t, r.Headers["host"])
